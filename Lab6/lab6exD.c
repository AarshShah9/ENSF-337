...
void String_append(String *dest, const String *source)
{
    ...
    char *new = malloc((size_t)(newSize + 1));

    int i, j = 0;
    for (i = 0; (dest->dynamic_storage[i]) != '\0'; i++, j++)
    {
        new[j] = dest->dynamic_storage[i];
    }

    for (i = 0; (source->dynamic_storage[i]) != '\0'; i++, j++)
    {
        new[j] = source->dynamic_storage[i];
    }
    new[j] = '\0';
    free(dest->dynamic_storage);
    dest->dynamic_storage = new;
    dest->length = newSize;
}
...
void String_truncate(String *dest, int new_length)
{
    ...
    char *new = malloc(((size_t)new_length) + 1);

    for (int i = 0; i < new_length; i++)
    {
        new[i] = dest->dynamic_storage[i];
    }
    new[new_length] = '\0';
    free(dest->dynamic_storage);
